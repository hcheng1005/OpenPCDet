...
'''
names: RadarPointFeatureEncoder
description: 毫米波雷达点云特征编码
return {*}
'''
class RadarPointFeatureEncoder(object):
    def __init__(self, config, point_cloud_range=None):
        super().__init__()
        self.radar_point_encoding_config = config
        assert list(self.radar_point_encoding_config.src_feature_list[0:3]) == ['x', 'y', 'z']
        self.used_feature_list = self.radar_point_encoding_config.used_feature_list
        self.src_feature_list = self.radar_point_encoding_config.src_feature_list
        self.point_cloud_range = point_cloud_range

    @property
    def num_point_features(self):
        return getattr(self, self.radar_point_encoding_config.encoding_type)(points=None)
    ...
    def forward(self, data_dict):
        ...
        # if self.radar_point_encoding_config.get('filter_sweeps', False) and 'timestamp' in self.src_feature_list:
        #     max_sweeps = self.radar_point_encoding_config.max_sweeps
        #     idx = self.src_feature_list.index('timestamp')
        #     dt = np.round(data_dict['points'][:, idx], 2)
        #     max_dt = sorted(np.unique(dt))[min(len(np.unique(dt))-1, max_sweeps-1)]
        #     data_dict['radar_points'] = data_dict['radar_points'][dt <= max_dt]
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        # print(points.shape[-1], len(self.src_feature_list))
        assert points.shape[-1] == len(self.src_feature_list)
        point_feature_list = [points[:, 0:3]] # 前三个特征固定为xyz
        for x in self.used_feature_list:

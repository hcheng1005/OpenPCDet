...
def parse_config():    
    data_path="/data/chenghao/mycode/private/OpenPCDet/data/nuscenes/v1.0-mini/samples/LIDAR_TOP"    
    cfg_ = "/data/chenghao/mycode/private/OpenPCDet/tools/cfgs/nuscenes_models/cbgs_voxel01_res3d_centerpoint.yaml"
    ckpt_ = "/data/chenghao/mycode/private/OpenPCDet/tools/ckpt/nuScenes/cbgs_voxel01_centerpoint_nds_6454.pth"
...
def main():
    ...
    if(1): 
        ...
        # 调用onnx-export进行模型导出
        output_path = "./about_nuscenes/export/centerpoint/output/"
        torch.onnx.export(  post_model, 
                            rpn_input, 
                            output_path +  "centerpoint_rpn_orin.onnx",
                            export_params=True, 
                            opset_version=11, 
        ...
        # 模型简化
        sim_model, check = simplify_model(output_path +  "centerpoint_rpn_orin.onnx")
        if not check:
            print("[ERROR]:Simplify %s error!"% "tmp.onnx")

        # 保存最后的onnx模型
        onnx.save(sim_model, output_path + "centerpoint_rpn.onnx")
